...
class GEX_PPI_GAT_cat4_MLP(nn.Module):  
    def __init__(self, ppi_adj, g2v_embedding, args):
        ...
        #   fix for serving scenario
        self.get_gex_idxs = []
        self.args = args


#    def forward(self, x):
    def forward(self, ecfp, gex, dosage, duration): 

        #   Serving Scenario
        training = False
        device = torch.device("cpu")
        args = self.args
        get_gex_idxs = self.get_gex_idxs
        ...
        drug_input = ecfp.float().to(device)
        #gex_input = gex[:, get_gex_idxs].float().to(device) # bs x n genes x 1
        gex_input = gex.float().to(device) # bs x n genes x 1
        dose = dosage.float().to(device).view(-1,1)
        duration = duration.float().to(device).view(-1,1)

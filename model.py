...
class GEX_PPI_SAGE_cat4_MLP(nn.Module):  
    def __init__(self, ppi_adj, g2v_embedding, args):
...
class GEX_PPI_HIGHER_cat4_MLP(nn.Module):  
    def __init__(self, ppi_adj, g2v_embedding, args):
        super(GEX_PPI_HIGHER_cat4_MLP, self).__init__()
        ...
        for i in range(args.num_gcn_hops):
            if i==0:
                self.gcns[i] = geo_nn.GraphConv(args.gene2vec_dim, args.gcn_hidden_dim1*4)
            else:
                self.gcns[i] = geo_nn.GraphConv(args.gcn_hidden_dim1*4, args.gcn_hidden_dim1*4)
        for i in range(args.num_gcn_hops):
            self.add_module('gcn_{}'.format(i), self.gcns[i])
